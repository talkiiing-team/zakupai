...
export type AssignTargetActionData = {
    type: 'assign_target';
    x: number;
};
...
type Props = {
    id: string;
    data: AssignTargetActionData;
};

export const AssignTargetNode: FC<Props> = ({ id, data }) => {
    ...
    return (
        <BaseNode
            title="Приравнять целевую метрику на X"
            color="gray"
        ...
        >
            <label className="flex flex-row items-center justify-center py-2">
                ...
                <input
                    className="nodrag w-[3ch] rounded-md border border-zinc-300 px-2 py-1"
                    type="text"
                    value={data.x}
                    onChange={onChange}
                />
